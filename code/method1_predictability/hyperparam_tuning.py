...
Within each config: checkpoint / early-stop on constructive val
  constructive = L_std + alpha * L_shared + lambda_ctr * L_ctr

Across configs: rank by fixed (unweighted) evaluation loss
  eval_loss = L_std + L_shared + L_ctr
...
# Metrics minimized when ranking (everything else is maximized)
...
def fixed_eval_loss(avg_terms):
    ...
    return float(avg_terms['l_std'] + avg_terms['l_shared'] + avg_terms['l_ctr'])
...
def summarize_best(metric=DEFAULT_RANK_METRIC):
    ...
    out = {
        ...
        'eval_loss_definition': 'L_std + L_shared + L_ctr (unweighted)',
        'config': best['config'],
    ...
    for i, r in enumerate(ranked[:10], 1):
        print(
            '{:2d}. {} | eval={:.4f} nmi={:.4f} ari={:.4f} acc={:.4f} knn={:.2f} constructive={:.4f}'.format(
                i,
                r['config'],
                float(r['eval_loss']),
                float(r['nmi']),
...
def train_one(batch, epochs, lr, wd, alpha, beta, gamma, lambda_ctr):
    ...
    for epoch in range(epochs):
        ...
        loss_val, constructive, avg_terms = validation(
            val_loader,
            view1_data,
            view2_data,
            model,
...
def load_hparams(path):
    ...
    # [lr, wd, alpha, beta, gamma, lambda_ctr]
    return {
        'lr': float(h[0]),
        'wd': float(h[1]),
        'alpha': float(h[2]),
...
def load_or_rebuild_val_data(path):
    ...
    # Older runs may only have model_*; rebuild with identical random_state splits.
...
def compute_eval_loss_from_checkpoint(path, batch=BATCH_SIZE, hparams=None):
    """Recompute fixed eval_loss for an existing checkpoint (resume path)."""
...
def run_sweep(args):
    ...
    print(
        'Rank metric default: {} = L_std + L_shared + L_ctr (unweighted); '
        'checkpoint/early-stop still uses constructive (weighted).'.format(DEFAULT_RANK_METRIC)
    )
    ...
    for idx, (alpha, beta, gamma) in enumerate(grid, 1):
        ...
        print(
            'RESULT {} | eval={:.4f} constructive={:.4f} nmi={:.4f} ari={:.4f} acc={:.4f} knn={:.2f}'.format(
                name,
                eval_loss,
                val_loss,
                metrics['nmi'],
