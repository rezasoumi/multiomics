"""Two-step training for Method 1 + VICReg ablation (g only as adversary)."""
...
def train(
    ...
    total_loss = 0.0
    total = 0.0
    sum_terms = {k: 0.0 for k in ('l_std', 'l_shared', 'l_adv', 'l_ctr', 'l_vic')}
    ...
    for train_batch in train_loader:
        ...
        h_rec, f_rec, g_rec = model.predict_all(specific_d, shared_d)
        out_step1 = {
            'specific': specific_d,
            'shared': shared_d,
        ...
        # Step 2: encoders / h / f / mlp (+ VICReg); freeze g
        _set_requires_grad(adv_params, False)
        specific, shared, shared_mlp = model.encode_views(view1, view2, view3)
        h_rec, f_rec, g_rec = model.predict_all(specific, shared)
        out_step2 = {
            'specific': specific,
            'shared': shared,
            'shared_mlp': shared_mlp,
    ...
    print(
        '\n [Epoch: %3d/%3d] Training Loss: %f | std=%.4f shared=%.4f adv=%.4f ctr=%.4f vic=%.4f'
        % (
            epoch + 1,
            total_epochs,
            res,
