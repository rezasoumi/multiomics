"""Evaluate Method 2 models (clustering + kNN)."""
...
from multiomics.code.method2_joint_vicreg.model import SharedAndSpecificEmbedding
...
def main(args):
    ...
    parent = '../../results/models_{}_method2'.format(disease)
    ...
    if args.config:
        ...
        if best['config'] is None:
            raise FileNotFoundError('No trained Method 2 models under {}'.format(parent))
        folder = best['config']
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Evaluate Method 2')
