"""Hyperparameter sweep for Method 2: λ_rec × λ_ctr × λ_vic (125 configs), rank by NMI."""
...
def summarize_best(metric='nmi'):
    ...
    for i, r in enumerate(ranked[:10], 1):
        print(
            '{:2d}. {} | nmi={:.4f} ari={:.4f} acc={:.4f} knn={:.2f} val={:.4f}'.format(
                i,
                r['config'],
                float(r['nmi']),
                float(r['ari']),
...
def train_one(batch, epochs, lr, wd, lambda_rec, lambda_ctr, lambda_vic):
    ...
    hist_fields = ['epoch', 'train_loss', 'val_loss', 'val_recon', 'val_ctr', 'val_vic']
...
def run_sweep(args):
    ...
    for idx, (lambda_rec, lambda_ctr, lambda_vic) in enumerate(grid, 1):
        ...
        print(
            'RESULT {} | val={:.4f} nmi={:.4f} ari={:.4f} acc={:.4f} knn={:.2f}'.format(
                name,
                val_loss,
                metrics['nmi'],
                metrics['ari'],
